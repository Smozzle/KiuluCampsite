...
<head>
  ...
  <title>ADMIN DELETE PRIZE</title>
...
<body>
  <?php
    if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET['id'])) {
      ...
      // delete prize record
      $sql = "DELETE FROM prizes WHERE luckyDrawID = ?";
      $stmt = mysqli_prepare($conn, $sql);
      mysqli_stmt_bind_param($stmt, "i", $prizeID);

      if (mysqli_stmt_execute($stmt)) {
        echo "
              <div id='userSuccessMessage'>
                <p>PRIZE WITH ID ($prizeID) DELETED SUCCESSFULLY!</p>
